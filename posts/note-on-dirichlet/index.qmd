...
In 1839, [Peter Gustav Lejeune Dirichlet](https://en.wikipedia.org/wiki/Peter_Gustav_Lejeune_Dirichlet) was a mathematician attached to the Philosophy department at the University of Berlin working for less than full pay even though he had become a member of the Prussian Academy of Sciences in 1832. To become a "full professor" at the university at that time it was required that a candidate deliver *his* *Habilitationsschrift* talk in Latin. Apparently, Dirichlet's facility with Latin wasn't up to the task, so like many "adjunct professors" today, Dirichlet took a side gig to support his family. He taught math at a military school.  Anyway, I digress. It was about that time that Dirichlet began to work on a problem in celestial mechanics which involved this integral:
...
When $\alpha_1 = \alpha_2 = \alpha_3 = constant$, the Dirichlet density is symmetric about the middle of the simplex. In the special case when $\boldsymbol{\alpha} = (1, 1, 1)$, the density is uniform over the simplex. When all the $\alpha_i < 1$ the density is concentrated at the vertices of the simplex, and when $\alpha_i > 1$, the density is concentrated in the center of the simplex.

## How the symetric Dirichlet distribution changes as $\alpha$ changes

The following animation, which projects the above plot onto two dimensions, shows how the Dirichlet distribution changes as the common value of $\alpha_i$, called the concentration parameter, moves systematically from (1,1,1), the uniform distribution, to (0.1,0.1,0.1). Generally values of the concentration parameter less than 1 lead to distributions concentrated at the vertices of the simplex while values greater than 1 lead to distributions with most of the mass concentrated on a few values. 
...
These next two plots of the first and last frames of the animation clearly show how the density moves from being uniformly distributed over the simplex to being concentrated at the vertices of the simplex. When modeling the development of a multi-state Markov chain as I was doing in the post I alluded to above, it might be reasonable to select a Dirichlet prior with $\boldsymbol{\alpha} = (1,1,1)$ if you had no prior information about the transition probabilities. However, if you had reason to believe that the process was likely to remain in the same state, then a prior with $\boldsymbol{\alpha} = (0.1,0.1,0.1)$ to start of the simulation with points concentrated near the sates.
...
This concentration of density as $\boldsymbol{\alpha}$ increases is very apparent in this next simulation as $\boldsymbol{\alpha}$ moves from (0.1, 0.1, 0.1)  to (10.0, 10.0, 10.0). Here we see the distribution concentrating on the mean which is (1/3, 1/3, 1/3).
...
Note that during this animation $\alpha$ passing through 0.5 which is the [Jeffreys prior](https://en.wikipedia.org/wiki/Jeffreys_prior) for the Dirichlet distribution. 
...
Differential entropy is defined for continuous probability distributions. It does not behave the same as the Shannon entropy which is used for discrete distributions. Among other things, differential entropy can be negative, in not invariant under a change of variables, and may not conform to any intuition you may have developed about maximum entropy. The plot on the left below shows the behavior of the differential entropy for the symmetric Dirichlet distribution we have been considering as $\alpha$ moves from 0.1 to 5.0. Note that the entropy keeps increasing beyond the point $\alpha = 1$ which corresponds to the uniform distribution over the simplex.
...
* Thomas M. Cover and Joy A. Thomas, *Elements of Information Theory Authors*, Wiley-Interscience Edition: 2nd Edition, (2006) 
