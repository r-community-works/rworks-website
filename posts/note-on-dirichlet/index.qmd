...
The [Dirichlet distribution](https://en.wikipedia.org/wiki/Dirichlet_distribution) with mean $E[X_i] = \alpha_i / \alpha_0$ and variance $Var[X_i] = \frac{\alpha_i (\alpha_0 - \alpha_i)}{\alpha_0^2 (\alpha_0 + 1)}$ brings together 18th and 19th century work in analysis as exemplified by the Gamma, Beta and digamma functions with early 20th ideas from geometry and topology (the simplex) and modern Bayesian statistics.

A simplex is a generalization of the notion of a triangle to multiple dimensions. The support, the points on which the distribution is defined, for a three dimensional Dirichlet distribution, is a (2)-simplex or triangular subset of a 2-dimensional plane intersecting the Euclidean axes at the points (1,0,0), (0,1,0), and (0,0,1). 
...
In K dimensions, the Dirichlet distribution is defined over the (K-1)-simplex, which is the set of K-dimensional vectors that sum to 1 and have non-negative components. So, in addition to being the support of the Dirichlet distribution, the simplex is also a natural way to represent probabilities that sum to 1 in multidimensional spaces.
...
This concentration of density is even more apparent in the this following simulation as $alpha$ moves from 0.1 to 5.0.
...
The entropy for the [Dirichlet distribution](https://en.wikipedia.org/wiki/Dirichlet_distribution) prominently displayed on the Wipipedia pageis given by the following equation:
...
This is differential entropy which is appropriate for continuous probability distributions. It does not behave the same as the Shannon entropy which is used for discrete distributions. Among other things, differential entropy can be negative, in not invariant under change of variables, and may not conform to any intuition you may have developed about maximum entropy. The plot on the left below shows the behavior of the symmetric Dirichlet distribution we have been considering as $\alpha$ moves from 0.1 to 5.0. Note that the entropy keeps increasing beyond the point $\alpha = 1$ which corresponds to the uniform distribution over the simplex.
...
Without attempting to waid through a whole bunch of difficult mathematics that I don't really understand, I think the way to interpret what the differential entropy is telling us is by considering the plot of the variance of the Dirichlet distribution as a function of the concentration parameter $\alpha$. As $\alpha$ increases, the variance goes to zero so that the distribution becomes concentrated about its mean which is apparent in the animations presented above. The result is that there are a lot more states which look alike and hence more uncertainty about which state the system is in. This is reflected in the increasing entropy.
