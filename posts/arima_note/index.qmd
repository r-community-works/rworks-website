...
Finally, note that I have compared the models according to their RMSE, but that the `ARIMA()` function selects models based on the AICc criterion. Comparing both of these metrics for the models computed by `fable` shows that these metrics would not select the same model.



```{r}
#Fetch AICc values for all models fit with fable
AICc_df <- data.frame(ARIMA_AICc <- glance(fit)["AICc"],
ARIMA_2_AICc <- glance(fit_2)["AICc"],
ARIMA_aA_AICc <- glance(fit_aA)["AICc"],
ARIMA_search_AICc <- glance(fit_search)["AICc"])
names(AICc_df) <- c("ARIMA", "ARIMA_2", "ARIMA_aA", "search")
AICc_df_longer <- AICc_df |> pivot_longer(cols = everything(), names_to = "model", values_to = "AICc")

#select faple RMSE values
rms_df <- data.frame(rms_fcst[2:5])

# Put in data frame to compare
compare_df <- cbind(AICc_df_longer, rms_df)
row.names(compare_df) <- NULL
names(compare_df) <- c( "Model", "AICc", "RMSE")
compare_df |> arrange(AICc)
