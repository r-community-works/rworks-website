...
image: "turtle.png"
image-alt: "ROC Curve drawn by a 'turtle' walking through specificity/sensitivity plane"
...
We reserve about 1/4 of cases for test set, and we’ll use the rest for training a predictive model. The plot shows the test set, since that is the data we’ll use to generate the ROC curves. If x is below about 20 all the points are red, and if it is above about 80 they are all black. In between is a region of varying uncertainty with more red at one end and more black at the other.
...
```{r roc_curve, fig.height=7, fig.width=7}
#| message: FALSE
#| warning: FALSE
#| code-fold: TRUE
...
```{r wandering_along_diagonal, fig.height=7, fig.width=7}
#| message: FALSE
#| warning: FALSE
#| code-fold: TRUE
set.seed(1)
