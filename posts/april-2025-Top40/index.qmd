...
[connector](https://cran.r-project.org/package=connector) v0.1.1: Facilitates cliniclresearch by providing a consistent interface for connecting to various data sources including file systems and databases including *ADAM* and *SDTM*. See the vignettes [connector](https://cran.r-project.org/web/packages/connector/vignettes/connector.html) and [How to extend connector](https://cran.r-project.org/web/packages/connector/vignettes/customize.html).

[verdata](https://cran.r-project.org/package=verdata) v1.0.0: Facilitates use and analysis of data about the armed conflict in Colombia resulting from the joint project between La Jurisdicción Especial para la Paz (JEP), La Comisión para el Esclarecimiento de la Verdad, la Convivencia y la No repetición (CEV), and the Human Rights Data Analysis Group (HRDAG). The data are 100 replicates from a multiple imputation through chained equations as described in [Van Buuren and Groothuis-Oudshoorn (2011)](https://www.jstatsoft.org/article/view/v045i03). See [README](https://cran.r-project.org/web/packages/verdata/readme/README.html) tp get started.
...
[assertHE](https://cran.r-project.org/package=assertHE) v1.0.0: Provides tools tohelp health economic modellers when building and reviewing models. The visualisation functions allow users to more easily review the network of functions in a project, and get lay summaries of them. The asserts included are intended to check for common errors. See [Smith et al. (2024)](https://wellcomeopenresearch.org/articles/9-701/v1) for details and [README](https://cran.r-project.org/web/packages/assertHE/readme/README.html) for examples.
...
[lsoda](https://cran.r-project.org/package=lsoda) v1.2: Provides an implementation of the `lsoda` function from the `ODEPACK` libraryefor solving initial value problems for first order ordinary differential equations [Hindmarsh (1982)](https://coemputing.llnl.gov/sites/default/files/ODEPACK_pub1_u88007.pdf) which can be called inline using `Rcpp`, and an `R` function,`ode`. Look [here](https://github.com/mclements/lsoda) for examples.
...
[hdMTD](https://cran.r-project.org/package=hdMTD) v0.1.0: Provides functions to estimates parameters in Mixture Transition Distribution (MTD) models, a class of high-order Markov chains. Estimated parameters include pasts (lags), transition probabilities and oscillations. A [perfect sampling algorithm](https://www.image.ucar.edu/staff/uli/DOWNLOAD/ps_notes.pdf) generates samples of an MTD model from its invariant distribution. See [Ost & Takahashi (2023)](http://jmlr.org/papers/v24/22-0266.html) for the theory and [README](https://cran.r-project.org/web/packages/hdMTD/readme/README.html) for examples.


[joker](https://cran.r-project.org/package=joker) v0.14.2: Extends he range of available distribution families and facilitates the computation of key parametric quantities, such as moments and information-theoretic measures. All package features are available both in a stats-like syntax for entry-level users, and in an S4 object-oriented programming system for more experienced ones. See the [vignette](https://cran.r-project.org/web/packages/joker/vignettes/joker.html).

[postcard](https://cran.r-project.org/package=postcard) v1.0.0: Provides functions to perfom power analysis and inference on marginal effects using plug-in estimation and influence functions that optionally leverage historical data to increase precision with prognostic covariate adjustment. The methods are described in [Højbjerre-Frandsen et al. (2025)](https://arxiv.org/abs/2503.22284). There are three vignettes including an [Introduction](https://cran.r-project.org/web/packages/postcard/vignettes/postcard.html) and [Prospective Power Estimation](https://cran.r-project.org/web/packages/postcard/vignettes/prospective-power.html).
