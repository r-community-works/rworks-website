...
> You have four (integral) weights $w_1, w_2, w_3, w_4$ and a balance scale such that you can weigh any (integral) number of pounds from 1 lb to 40 lbs. The weights may go on either side of the scale (eg, with the object, or in the opposite pan). What are the $w_i$?
...
We can calculate that $3^4 = 81$, which corresponds to $(81 - 1)/2 = 40$ positive values (surprise!). Therefore
we have that $w_4 = 40 - (1 + 3 + 9) = 27$. And so the set of weights we want is (1, 3, 9, 27), as stated above.
