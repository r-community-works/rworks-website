...
date: 20256-03-15
description: "This post presents a proof of concept for Healthcare Economics Models constructed from a patient's perspective. Using inputs derived from the literature, it builds an eight state continuous time Markov chain to construct a synthetic cohort of patients undergoing treatment for stage II squamous cell carcinoma. The total expected time in each treatment health state for two treatment arms are computed from and paired with EQ-5D utilities derived from the literature. The resulting health economics model compares the QALYs associated with each treatment arm. It is envisioned that future models of this sort constructed under the supervision of physicians and health care professionals would be useful both to clinicians and patient care teams both in formalizing and documenting the treatment decision process and in communicating with patients."
editor: source
format: html
---


Most health care economics models are constructed from the perspective of a managed health care system such as those offered in Canada and several European countries, or from the perspective of some other third party such as an insurance company. Although the benefits of constructing models from the patient's perspective have been discussed in the literature, see for example [Ioannidis & Garber (2011)](https://pmc.ncbi.nlm.nih.gov/articles/PMC3134464/) and [Tai et al. (2016)](https://pubmed.ncbi.nlm.nih.gov/27712720/), few models have been published. Obvious reasons for this situation would include the privacy issues associated with obtaining patient specific data and the apparent lack of economic incentives to commit resources required to abstract individual patient trajectories from patient level data models.

This post explores the hypothesis that detailed, data-driven, patient-specific models may not be necessary in order to achieve most of the benefits of a health economics model from the patients perspective. I present a lightweight, cohort model that I believe captures the majority of the benefits to both physicians and patients that might result from a patient focused model.
...
The model begins with the decision to select radiation or surgery and identifies eight health states that patient will pass through. The states visited and the order of visiting them depend on the initial decision. The next step uses the theory of continuous time Markov chains (CTMC) to construct a synthetic data set for a cohort of patients who vary in age and tumor size. The CTMC model is driven by The estimates for the jump-chain transition probabilities and mean state occupancy times. I have chosen this approach because I believe that these quantities will have intuitive appeal to clinicians and medical experts. I have attempted to abstract reasonable estimates for these parameters from the medical literature. If done correctly, these sorts of abstracted estimates should be helpful as default parameters which clinicians could change based on their experience and intuition. 

The key insight here is that the synthetic data set is the underlying statistical model. Empirical survival curves and the time spent in each state and other useful quantities are then calculated from the synthetic data. The patient specific healthcare model is then constructed by estimating the utility of each health state to the patient. This is accomplished by constructing quality adjust life years (QALYs) based on [EQ-5D](https://www.ncbi.nlm.nih.gov/books/NBK565680/) values reported in the literature. 
...
The first plot shows overall survival (Kaplan-Meier) curves for patients in both treatment arms and the overall survival curves for the individual survival curves. Note that, as I believe would be expected, surgery exhibits a slightly better overall survival curve (Approximately 73% vs approximately 65% at 5 years). However, both curves show very good five year survival probabilities. 
...
The second plot panel shows the overall survival curves by treatment for each of the four possible values of the covariates.
...
Note that the covariate effects are encoded in the `make_Q` function used to create the synthetic data as follows:
