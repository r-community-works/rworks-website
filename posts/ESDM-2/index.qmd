...
The following figure depicts the decision as a tree. The square node shows the decision (whether or not to administer the antibiotic) and the round nodes show the subsequent chance event of wound infection. The two branches represent the treatment group (trt) who receive the antibiotic and the control group (ctl), who do not. The probability that a patient in the control group gets a wound infection is p1. In the treated group we expect the probability of infection (p2) to be different from the control group. The four leaves of this tree show the costs associated with the four outcome categories. The leaf labels ('cost_wdpx', etc) will be variables in our models, where 'wd' means the surgical wound is infected, 'nwd' means it is not, and 'px' means the patient was prescribed the prophylactic antibiotic. The costs of the leaves will be weighted by the probabilities to compute the expected payoff of each alternative for the decision. To keep things simple for now we only consider monitary costs; later we will generalize for utility (QALYs)
...
Having established a deterministic baseline model, the next step is to account for the uncertainty in the modeling assumptions by developing a Bayesian stochastic model that uses probability distributions rather than point estimates to represent uncertain parameters. These are shown in red in the diagram below.
...
**<< BOB TO DO: Paragraph about why (and how) to use utility (QALYs) instead of just monetary cost.>>**

**<< BOB TO DO: Paragraph on computing INB at M different lambda values. Describe 'step' function. >>**
