...
In this first post in a series of two, I will introduce a complex healthcare data set and outline the problems I propose to solve using `tidyverse` and [GitHub Copilot](https://docs.posit.co/ide/user/ide/guide/tools/copilot.html), which will facilitate nuanced argument tuning in `tidyverse` functions and accelerate our data analysis process.
...
Adding Copilot as a pair programmer to your RStudio is simple, and has a seamless interface. Take a look at the documentation [**here**](https://docs.posit.co/ide/user/ide/guide/tools/copilot.html).
...
![](images/admission_type_updated.png){fig-align="center" style="box-shadow: 5px 5px 15px rgba(0, 0, 0, 0.3); border-radius: 5px;"}
...
![](images/diag_1.png){fig-align="center" style="box-shadow: 5px 5px 15px rgba(0, 0, 0, 0.3); border-radius: 5px;"}
...
We see that the ICD9 values for a respiratory diagnosis check out in the output. Also note there is no code $460$ (a common cold diagnosis).
...
![](images/missing_diag.png){fig-align="center" style="box-shadow: 5px 5px 15px rgba(0, 0, 0, 0.3); border-radius: 5px;"}
...
1.  In plot 1, it's useful to see the missing value pattern, via `NULL`, `Not Available` and `Not Mapped` values. Also, by plotting `admission_type` against `age`, we can see encounters where the data was the most dense, e.g., emergency admissions for the $70-80$ year age group.
...
![](images/autocomplete.png){fig-align="center" style="box-shadow: 5px 5px 15px rgba(0, 0, 0, 0.3); border-radius: 5px;"}
