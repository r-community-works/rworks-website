...
[mmbcv](https://cran.r-project.org/package=mmbcv) v0.3.0: Provides a fucntion to compute a  robust and bias-corrected sandwich variance estimators for multi-state Cox models with clustered time-to-event data. The methodology extends the marginal Cox model bias-correction framework of [Wang et al. (2023)](https://onlinelibrary.wiley.com/doi/10.1002/bimj.202200113) to the multi-state setting. See the [vignette](https://cran.r-project.org/web/packages/mmbcv/vignettes/mmbcv-intro.html).
...
[HeatmapR](https://cran.r-project.org/package=HeatmapR) v1.0.0: Provides a lightweight framework for creating high quality, complex heatmaps using base graphics. Supports hierarchical clustering with dendrograms, column and row scaling, cluster sub-divisions, customizable cell colours, shapes and sizes, legends, and flexible layouts for arranging multiple heatmaps. Designed to return plot objects that can be easily arranged with other plots without sacrificing resolution. Methods for hierarchical clustering and distance computations are described in [Murtagh and Contreras (2012)](https://wires.onlinelibrary.wiley.com/doi/10.1002/wics.53). Dendrogram visualisation methods are based on the `ggdendro` package by [de Vries and Ripley (2020)](https://CRAN.R-project.org/package=ggdendro). See the [vignette](https://cran.r-project.org/web/packages/HeatmapR/vignettes/HeatmapR.html).
