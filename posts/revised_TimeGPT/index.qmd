...
This next section of code reformat the training data into a format that is suitable for the `forecast::auto.arima()` and `forecast::auto.ets()` functions. Both of these functions require that the data be expressed as a `ts()` object. The original version of this post formatted the data as an `xts()` object: an error that substantially impacted the ARIMA and exponential smoothing forecasts. The code below creating the `ts()` object treats time as an index of days with a period of 24. The electricity data has multiseasonal aspects and this treatment of time just throws away most of the information associated with years and months. However, `ts()` was not designed to deal with multiseasonal data and my intention here is to compare `TimeGPT` with a couple of standard automated forecasts, not expertly tuned models.
...
elec_ts <- ts(auto_train$y, start = 2016+295/365 , frequency = 24)
...
## Acknowledgment
Many thanks to Professor Rob Hyndman for flagging the ARIMA time series object error described above and for generously providing information on alternative models which I hope to use in future posts. Any errors still remaining in this post are all mine.
