...
date: 2026-09-29
description: "The CRAN Top 40 is an attempt to sort through the massive amount of new packages submitted to CRAN every month and find forty, `quality` packages that demonstrate the broad use of the R language, and appeal to a wide audience of potential users. If you take the time to scan through the post I hope you will find something interesting and maybe even useful."
image: "gbif.png"
image-alt: "Map showing 10 species classes over the European Alps, using two CHELSA bioclimatic layers"
categories: "Top 40"
editor: source
format: html
---
...
![](gbif.png){fig-alt="Map showing 10 species classes over the European Alps, using two CHELSA bioclimatic layers"}
...
[neuralsbi](https://cran.r-project.org/package=neuralsbi) v0.3.2: Provides a native R implementation of a neural simulation-based estimator that runs on the `torch` back end and is focused on Neural Posterior Estimation. Given a prior over parameters and a simulator, `neuralsbi` trains a conditional neural density estimator to approximate the Bayesian posterior, enabling amortized, likelihood-free inference. It targets applied researchers who want an approachable interface with sensible defaults and built-in posterior diagnostics. There are four vignettes including [Getting started](https://cran.r-project.org/web/packages/neuralsbi/vignettes/neuralsbi.html) and [Case study: inferring epidemic parameters (SIR)](https://cran.r-project.org/web/packages/neuralsbi/vignettes/sir-epidemic.html).

![](neuralsbi.png){fig-alt="Plots showing fit of multi-modalestimator"}
...
[orthoMTL](https://cran.r-project.org/package=orthoMTL) v0.1.0: Fits regularised multi-task learning models where relationships between tasks are controlled via orthogonality or disjoint-support constraints. Supports regression, binary classification, and censored survival data. In survival mode, time-to-event outcomes are converted into binary labels at user-defined thresholds, enabling the discovery of features with time-varying effects that standard proportional-hazards models cannot detect. Implements the penalty described in [Vervier et al. (2014)](https://hal.science/hal-00985654). See the [vignette](https://cran.r-project.org/package=orthoMTL).

![](orthoMTL.png){fig-alt="Heatmap showing coefficient weights across time thresholds"}
