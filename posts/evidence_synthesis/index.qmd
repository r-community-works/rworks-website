...
Here we read the data from a local Excel file. Note that the Blocker Excel file is among the supporting materials that can be downloaded forom the [Wiley site](https://bcs.wiley.com/he-bcs/Books?action=resource&bcsId=7268&itemId=047006109X&resourceId=28154).
...
# Note thaat the WinBugs code in the book initializes the MCMC algorithm with a single chain as follows.
...
A key MCMC dianostic reported by the `jags() function` of the jagsUI package is sufficient.adapt, a logical value indicating whether the number of iterations for adaptation was sufficient. For this model:
