...
date: 2026-04-28
description: "An idiosyncratic, unabashedily biased, time constrained attempt to capture the depth and breath of the new packeges submitted to CRAN in a single month entirely without the use of AI: clearly unsustainable, but maybe entertaining and somewhat useful."
...
[opensimplex2](https://cran.r-project.org/package=opensimplex2) v0.0.3: Provides a function to generate 2, 3 or 4-dimensional gradient noise. The noise function is comparable to classic Perlin noise, but with less directional artefacts and lower computational overhead. It can have applications in procedural generation or flow fields simulations. See the [vignette](https://cran.r-project.org/web/packages/opensimplex2/vignettes/opensimplex-on-fire.html).
...
[mmbcv](https://cran.r-project.org/package=mmbcv) v0.3.0: Provides a function to compute a  robust and bias-corrected sandwich variance estimators for multi-state Cox models with clustered time-to-event data. The methodology extends the marginal Cox model bias-correction framework of [Wang et al. (2023)](https://onlinelibrary.wiley.com/doi/10.1002/bimj.202200113) to the multi-state setting. See the [vignette](https://cran.r-project.org/web/packages/mmbcv/vignettes/mmbcv-intro.html).
...
[BayesVolcano](https://cran.r-project.org/package=BayesVolcano) v1.0.1: Bayesian models are used to estimate effect sizes (e.g., gene expression changes, protein abundance differences, drug response effects) while accounting for uncertainty, small sample sizes, and complex experimental designs. However, Bayesian posteriors of models with many parameters are often difficult to interpret at a glance. One way to quickly identify important biological changes based on frequentist analysis are volcano plots (using fold-changes and p-values). Bayesian volcano plots bring together the explicit treatment of uncertainty in Bayesian models and the familiar visualization of volcano plots. See [Sousa (2020)](https://www.sciencedirect.com/science/article/abs/pii/S0003267019313492?via%3Dihub) for background and the [vignette](https://cran.r-project.org/web/packages/BayesVolcano/vignettes/BayesVolcano.html) for examples.
